...
In this adventure game, players control a dog character and explore different areas to uncover hidden bones buried beneath the surface. Using movement and exploration skills, players must search carefully to find all the hidden treasures while overcoming challenges. The game focuses on problem-solving, exploration, and discovery.

- Control a dog and dig for hidden buried bones.
- Bones are invisible on the map. Use the proximity indicator to find them.
- Win by collecting 10 bones. Touching enemies (cats) costs a life.
...
```bash
open file:///Users/apple/CSP_python_work/hackathon_project/1/bone_hunt.html
```
